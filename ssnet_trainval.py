...
class ssnet_trainval(object):
  ...
  def initialize(self):
    ...
    print("MINIBATCH_SIZE", self._cfg.MINIBATCH_SIZE)
    print(filler_cfg)
    time.sleep(1) 
    ...
    dim_label = self._input_main.fetch_data(self._cfg.KEYWORD_LABEL).dim()
    #dims = []
    data_dims = []
    label_dims = []
    self._net = uresnet(data_dims=dim_data[1:],
                        label_dims=dim_label[1:],
  ...
  def ana_step(self,batch_mode=False):
    ...
    if not batch_mode:
      ...
      #copy_entries  = np.sort(copy_entries, axis=None)
      print(copy_entries)
      #print(np.sort(copy_entries, axis=None))
